...
function safeFloat(value, defaultValue = 0.0) {
  if (value == null || Number.isNaN(parseFloat(value))) return defaultValue
...
export function calculateMetrics(data) {
  ...
  const devops_hours = safeFloat(data.devops_hours)
  const devops_hour_cost = safeFloat(data.devops_hour_cost)
  const capex_devops = devops_hours * devops_hour_cost
  ...
  const total_capex = capex_dev + capex_devops + capex_third_party
  ...
  if (time_spent_hours > 0 && development_estimate_hours > 0) {
    const capex_real = (time_spent_hours * tech_hour_cost) + capex_devops + capex_third_party
    if (capex_real > 0) {
  ...
  return {
    ...
    capex_devops_cost: capex_devops,
