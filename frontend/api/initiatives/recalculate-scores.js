...
export default async function handler(req, res) {
  ...
  try {
    ...
    return res.status(200).json({
      message: `Scores recalculados para ${updated} iniciativas.`,
      count: updated,
      errors
    })
  } catch (err) {
    console.error('[initiatives/recalculate-scores.js]', err)
    return res.status(500).json({ error: err.message || 'Erro ao recalcular scores' })
  }
}
